...
class ServerObject(object):
        ...
        try:
            for i in self.client:
                if i.estConnecte and i.temps+self.maxTempsDecalage < self.client[num].temps:
                    return None
            lowAction = self.getLowestActionTime()
            if self.actions and self.client[num].temps-self.maxTempsDecalage > lowAction:
                self.deleteLowest(lowAction)
    ...
    def getLowestActionTime(self):
        return min([int(clee) for clee in self.actions])

    def getHighestActionTime(self):
        return max([int(clee) for clee in self.actions])

    def deleteLowest(self,lowAction): # cherche le client qui est le plus en retard dans la lecture des evenement
        try:
            lowClient = self.getLowestRead()
            if lowClient > lowAction:    #Traduction du if en français : si le plus petit temps des clients connecté est plus grand que le temps de la plus vielle action, on supprime cette action
                if(self.test):print("_______Suppression",self.actions[1])
                del self.actions[lowClient]     # on enleve levenement le plus bas
        except KeyError:
            print(traceback.print_exc())
